...
param_grid = {
    ...
    "regul": ["l1", "l2"],
    "regulrate": [0.00001, 0.001, 0.01],
    "batchsize": [16, 32, 64],
    "RMSPropconst": [1e-6, 1e-5, 1e-4],
    "RMSProprate": [0.8, 0.9, 0.99]
}

# --- Alle Kombinationen generieren ---
keys, values = zip(*param_grid.items())
combinations = [dict(zip(keys, v)) for v in itertools.product(*values)]
print(f"Gesamtanzahl der RMSProp-Durchläufe: {len(combinations)}")
...
for i, params in enumerate(combinations, 1):
    ...
    exe = MLP(
        ...
        adagradconst=10e-7,
        RMSProp=True,
        RMSPropconst=params["RMSPropconst"],
        RMSProprate=params["RMSProprate"],
        Adam=False,
...
df.to_csv("gridsearch_RMSProp_results.csv", sep=";", index=False)
print("\n✅ Ergebnisse gespeichert in 'gridsearch_RMSProp_results.csv'")
